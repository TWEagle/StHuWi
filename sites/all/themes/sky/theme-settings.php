...
function sky_form_system_theme_settings_alter(&$form, &$form_state) {
  ...
  if (at_get_setting('enable_extensions') === 1) {
    $form['at']['corners'] = array(
      ...
      '#description' => t('<h3>Rounded Corners</h3><p>Rounded corners are implemented using CSS and only work in modern compliant browsers. You can set the radius for both the main content and main menu tabs.</p>'),
    );
