/** * @file * Code based on Google Map APIv3 Tutorials. */
...
function if_gmap_init(id) {
  ...
  return false;
}/** * Map on submissions page. */function if_gmap_init_submissions(id) {  var map = document.getElementById(id + "_elementform_id_temp");  var def_zoomval = parseInt(map.getAttribute("zoom"));  var def_longval = map.getAttribute("center_x");  var def_latval = map.getAttribute("center_y");  var curpoint = new google.maps.LatLng(def_latval, def_longval);  if (gmapdata[id]) {    // If second time click to open popup Show on Map in submissions.    var second_map_div = document.getElementById("second_map_div");    if (second_map_div) {      // Remove the previous map div.      second_map_div.removeChild(map);      // Append map div to parent div.      second_map_div.appendChild(gmapdata[id].getDiv());      // Change the marker position.      gmapdata[id].panTo(curpoint);    }  }  else {    gmapdata[id] = new google.maps.Map(document.getElementById(id + "_elementform_id_temp"), {      center:curpoint,      zoom:def_zoomval,      mapTypeId:'roadmap'    });  }  google.maps.event.addListener(gmapdata[id], 'zoom_changed', function () {    document.getElementById(id + "_elementform_id_temp").setAttribute("zoom", gmapdata[id].getZoom());  });  gmapmarker[id] = new Array();  return false;}
